...
Application tables cover `procesoelectoral`, `publicoobjetivo`, `tipoactividad`, `formatoactividad`, `tipoasistentes`, `juradonacionalespecial`, `registroactividad`, and `registroactividadparticipante`. Preserve the original string-based date (`YYYY-MM-DD`) and time (`HH:MM`) values as parsed `date` and `time` fields; retain audit timestamps and soft-active state. Electoral processes identify a single active default for new registrations; edit flows retain the process already stored on the activity. Keep jury source process codes as integer metadata. Do not require a jury/process match on new registrations until an authoritative mapping exists. The empty `parametro` table needs no production seed, but an import inventory records it.
...
Monitor is the imported administrative role: global registration access plus the active seeded actions for configuration and user management. Gestor has registration list/add/edit/delete actions only and sees or changes registrations it created. Both source roles can read the active catalogs needed to enter a registration. A separate institution-specific Administrator role can be assigned to an explicitly designated Monitor without changing the reconciled source matrix; it grants every active module/action pair, including permission administration, while Monitor continues to provide global RLS scope. A protected permission helper reads active profile, role, module, and action records; RLS combines that action grant with creator ownership for Gestor. No `profile_jury_assignments` table is created. Protect the final active Monitor administrator from demotion or disablement. Role administration is performed through a narrow authenticated administrative function or trusted server flow; arbitrary browser writes to membership tables are denied.
...
Angular authentication moves to Supabase Auth. Repositories call RLS-safe tables, invoker views, narrow RPCs, and the `admin-directory` Edge Function through `SupabaseService`, mapping snake_case fields to the existing Spanish interfaces. Registration listing keeps active-only rows, pagination, code search, optional JEE filtering, and the actor's permitted creator scope. Module/action permissions drive navigation; the permission RPC includes authorized hierarchy-only parents so the header retains the legacy Administración and Seguridad groups, while non-functional modules without routes remain hidden. Direct database requests remain protected independently of the UI. Participant DNI autocomplete reuses the latest accessible imported participant record through the same RLS scope; unknown DNIs remain available for manual entry without calling IONOS. User and role administration runs through the authenticated Edge Function, which checks live action grants before using server-only Auth administration and service-role-only transactional RPCs. New administrative accounts remain at `pending_invitation` with an unknown random password until the owner approves onboarding. The legacy proxy and IONOS URL are removed after the administrative flow passes parity verification.
